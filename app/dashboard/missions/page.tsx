      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-3 text-sm text-text-muted max-w-2xl">
            Retrouvez l’historique de vos interventions, accédez aux missions en
            cours et suivez les prochaines dates.
          </p>
        </div>
        <Link
          href="/dashboard"
      ...
      <section className="card p-8 bg-white border border-border rounded-4xl shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <div className="rounded-2xl bg-bg-alt p-4 text-center">
              ...
              <p className="text-xl font-black text-primary-dk">
                {missions?.filter((m: Mission) => m.status === "confirmed")
                  .length ?? 0}
              </p>
            </div>
            <div className="rounded-2xl bg-bg-alt p-4 text-center">
              ...
              <p className="text-xl font-black text-primary-dk">
                {missions?.filter((m: Mission) => m.status === "pending")
                  .length ?? 0}
              </p>
            </div>
            <div className="rounded-2xl bg-bg-alt p-4 text-center">
              ...
              <p className="text-xl font-black text-primary-dk">
                {missions?.filter((m: Mission) => m.status === "completed")
                  .length ?? 0}
              </p>
            </div>
          </div>
        ...
        {error ? (
          <div className="rounded-4xl bg-red-50 border border-red-200 p-6 text-red-700">
            Impossible de récupérer les interventions. Vérifiez la configuration
            Supabase.
          </div>
        ) : !missions || missions.length === 0 ? (
          <div className="rounded-4xl bg-bg-alt p-8 text-center text-text-muted">
            Aucune mission trouvée pour le moment. Commencez par accepter une
            demande depuis votre espace client.
          </div>
        ) : (
          <div className="space-y-4">
            {missions.map((mission: Mission) => (
              <div
                key={mission.id}
                className="rounded-4xl bg-white border border-border p-6 shadow-sm hover:border-primary/20 transition-colors"
              >
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div>
                    ...
                    <p className="text-sm text-text-muted">
                      {mission.location} • {mission.customer_name}
                    </p>
                  </div>
